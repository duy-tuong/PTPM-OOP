...
// Đợt 13, Phần 1 (A2) - khách tự huỷ đơn CHƯA thanh toán của chính mình, gọi từ
// app/api/order-requests/[id]/cancel/route.ts.
export function cancelOrderRequest(orderId: number, token: string) {
  return apiFetch<OrderRequestDto>(getApiUrl(), `/order-requests/${orderId}/cancel`, "POST", { token });
}
