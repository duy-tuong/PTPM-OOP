...
export default async function AdminAuditLogsPage({ searchParams }: AdminAuditLogsPageProps) {
  // GET /admin/audit-logs chỉ [Authorize(Roles="Admin")] - chặn trước khi gọi API, đồng bộ với các
  // trang Admin-only khác (khác Dashboard vốn vẫn còn nội dung dùng được cho Editor).
  ...
  const logs = await getAdminAuditLogs(baseUrl, {
    pageNumber,
    pageSize: 20,
