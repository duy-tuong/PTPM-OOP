...
public class AuditLogConsultationObserverTests
{
    [Fact]
    public async Task OnStatusChangedAsync_AddsAuditLogWithExpectedFields_AndDoesNotSaveChanges()
    {
        ...
        var sut = new AuditLogConsultationObserver(unitOfWorkMock.Object);
        var changedByUserId = Guid.NewGuid();

        await sut.OnStatusChangedAsync(42, ConsultationStatus.New, ConsultationStatus.Contacted, changedByUserId);

        repositoryMock.Verify(r => r.AddAsync(It.Is<AuditLog>(log =>
            log.UserId == changedByUserId &&
            log.Action == AuditAction.StatusChange &&
            log.EntityName == nameof(ConsultationRequest) &&
            log.EntityId == "42" &&
            log.OldValues == nameof(ConsultationStatus.New) &&
            log.NewValues == nameof(ConsultationStatus.Contacted)),
