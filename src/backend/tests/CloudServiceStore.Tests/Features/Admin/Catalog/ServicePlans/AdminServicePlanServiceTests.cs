...
public class AdminServicePlanServiceTests
{
    ...
    public async Task GetListAsync_ReturnsAllPlans_IncludingInactive()
    {
        ...
        context.ServicePlans.AddRange(
            new ServicePlan { Id = 601, CategoryId = category.Id, Name = "Active Plan", Slug = "active-plan", IsActive = true },
            new ServicePlan { Id = 602, CategoryId = category.Id, Name = "Inactive Plan", Slug = "inactive-plan", IsActive = false });
        await context.SaveChangesAsync();
        ...
        Assert.Contains(result.Items, p => p.Slug == "inactive-plan");
    }
    ...
    public async Task GetByIdAsync_Found_ReturnsPlanWithFeaturesAndPrices()
    {
        using var context = TestDbContextFactory.CreateContext();
        var category = await SeedCategoryAsync(context);
        var plan = new ServicePlan { Id = 603, CategoryId = category.Id, Name = "Test Plan", Slug = "test-plan-asp", IsActive = true };
